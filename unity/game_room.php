...
// Log the request
error_log("=== GAME ROOM REQUEST (Unity) ===");
...
function pollActions() {
    ...
    // Unity compatibility: Convert response_data to JSON string and decode request_data
    foreach ($actions as &$action) {
        if (!empty($action['response_data'])) {
            $decoded = json_decode($action['response_data'], true);
            if (json_last_error() === JSON_ERROR_NONE) {
                // Unity compatibility: Keep as JSON string
                $action['response_data_json'] = json_encode($decoded, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
...
function getPendingActions() {
    ...
    // Unity compatibility: Convert request_data to JSON string
    foreach ($actions as &$action) {
...
function pollUpdates() {
    ...
    // Unity compatibility: Convert data_json back to object for each update
    foreach ($updates as &$update) {
        $decoded = json_decode($update['data_json'], true);
        if (json_last_error() === JSON_ERROR_NONE) {
            // Keep as JSON string for Unity compatibility
...
try {
    ...
    error_log("Critical error in game_room.php (Unity): " . $e->getMessage());
