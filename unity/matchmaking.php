...
// Log the request
error_log("=== MATCHMAKING REQUEST (Unity) ===");
...
function listMatchmaking() {
    ...
    try {
        ...
        // Unity compatibility: Convert extra_json_string to JSON string for each lobby
        foreach ($lobbies as &$lobby) {
            if (!empty($lobby['extra_json_string'])) {
                $decoded = json_decode($lobby['extra_json_string'], true);
                $lobby['extra_json_string'] = json_encode($decoded, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
            } else {
                $lobby['extra_json_string'] = '{}';
            }
        }
...
function createMatchmaking() {
    ...
    $extraJsonString = isset($data['extraJsonString']) ? json_encode($data['extraJsonString'], JSON_UNESCAPED_UNICODE) : '{}';
...
function getCurrentMatchmakingStatus() {
    ...
    try {
        ...
        // Unity compatibility: Convert extra_json_string to JSON string
        $extraJson = '{}';
        if (!empty($matchmaking['extra_json_string'])) {
            $decoded = json_decode($matchmaking['extra_json_string'], true);
            $extraJson = json_encode($decoded, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        }
        ...
        sendResponse([
            ...
            'matchmaking' => [
                ...
                'extra_json_string' => $extraJson, // Unity compatibility
...
function startMatchmaking() {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            ...
            VALUES (?, ?, ?, ?, ?)
        ");
        $stmt->execute([$roomId, $matchmaking['game_id'], $roomName, $matchmaking['max_players'], $matchmakingId]);
...
try {
    ...
    error_log("Critical error in matchmaking.php (Unity): " . $e->getMessage());
