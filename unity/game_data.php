...
try {
    ...
    $path = str_replace('unity/game_data.php', '', $path);
    $path = ltrim($path, '/');
    $path = rtrim($path, '/');
    ...
    switch ($method) {
        case 'GET':
            ...
            if ($path === 'game/get') {
                ...
                // Unity compatibility: Convert data to JSON string instead of object
                sendResponse([
                    'success' => true,
                    'type' => 'game',
                    'game_id' => $game['id'],
                    'data_json' => json_encode($gameData, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)
                ]);
            }
            ...
            elseif ($path === 'player/get') {
                ...
                sendResponse([
                    ...
                    'data_json' => json_encode($playerData, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)
                ]);
            }
            else {
